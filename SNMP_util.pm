...
@EXPORT =
  qw(snmpget snmpgetnext snmpwalk snmpset snmptrap snmpgetbulk snmpmaptable snmpmaptable4 snmpwalkhash snmpmapOID snmpMIB_to_OID snmpLoad_OID_Cache snmpQueue_MIB_File);
        ...
        or ( $SNMP_util::IPv4only ne $v4onlystr ) )
    {
        ...
        $SNMP_util::Session =
          ...
          : SNMP_Session->open( $host, $community, $port, undef,
            $lport, undef, $lhost, ( $v4onlystr eq 'v4only' ) ? 1 : 0 );
        (
            $SNMP_util::Host     = $nhost,
            $SNMP_util::Version  = $version,
            $SNMP_util::LHost    = $nlhost,
            $SNMP_util::IPv4only = $v4onlystr
        ) if defined($SNMP_util::Session);
    }
...
sub snmpwalk_flg ($$@) {
    ...
    while (
        ...
        ? $session->getbulk_request_response( 0,
            $session->default_max_repetitions(), @nnoid )
...
sub snmpmaptable4($$$@) {
    ...
    return $session->map_table_start_end(
        [@enoid],
        sub() {
            my ( $ind, @vals ) = @_;
            my ( @pvals, $val );
